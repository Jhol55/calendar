...
export default function Index() {
  ...
  useEffect(() => {
    ...
    const fetchInstances = async () => {
      InstanceService.getInstances(user?.id).then(({ data }) => setInstances(data));
    }
    ...
    <main className="relative flex items-center w-full max-w-screen h-screen">
      ...
      <section className="flex flex-col items-center justify-center h-full mx-auto w-full">
        <div className="flex items-center gap-4 w-2/3 mb-6">
          ...
          <Button className="w-fit" variant="gradient" ref={buttonRef} onClick={() => setShowCreateInstanceForm(prev => !prev)}>Adicionar</Button>
        </div>
      ...
      <AnimatePresence>
        {showCreateInstanceForm && (
          ...
          >
            <CreateInstanceForm onSubmitSuccessful={() => setTimeout(() => setShowCreateInstanceForm(false), 2000)} />
          </motion.div>
        )}
      </AnimatePresence>
